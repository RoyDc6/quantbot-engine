...
def _atr(h, l, c, n=14):
    ...
    if len(c) >= n:
        r[n-1] = np.mean(tr[:n])
        for i in range(n, len(c)): r[i] = (r[i-1]*(n-1)+tr[i])/n
    return r
...
def extract_factors(df, max_bars=1500):
    """从K线DataFrame提取43个候选因子"""
    ...
    for period, name in [(5,'bb_pos_5'),(10,'bb_pos_10'),(20,'bb_pos_20')]:
        ...
        f[name] = np.zeros(n)
        for i in range(period-1, n):
            f[name][i] = (o[i]-ma[i])/(std[i]*2+1e-10) if std[i] > 1e-10 else 0
    ...
    vol_ma5 = _ma(ov, 5)
    vol_surge = np.array([ov[i]/max(vol_ma5[i],1) if i>=4 and vol_ma5[i]>1e-6 else 1.0 for i in range(n)])
    f['vol_surge'] = vol_surge
    ...
    for d in [3,5,10,20,60]:
        m = np.full(n, 0.0)
        if n > d:
            m[d:] = (o[d:]-o[:-d])/np.maximum(o[:-d], 1e-10)
        f[f'mom_{d}d'] = m

    mom_accel = np.full(n, 0.0)
    for i in range(10, n):
    ...
    # ── 5. 多周期 ────────────────────────────────────
    weekly_c = np.array([o[i*5+4] if i*5+4<n else o[-1] for i in range(n//5)])
    wrsi = _rsi(weekly_c, 4) if len(weekly_c)>=4 else np.full(len(weekly_c), 50)
    monthly_c = np.array([o[i*20+19] if i*20+19<n else o[-1] for i in range(n//20)])
    ...
    f['weekly_rsi'] = w_rsi
    f['monthly_rsi'] = m_rsi
    res_d = np.where(f['rsi_14']<30, 1, np.where(f['rsi_14']>70, -1, 0))
    res_w = np.where(w_rsi<30, 0.5, np.where(w_rsi>70, -0.5, 0))
    res_m = np.where(m_rsi<30, 0.25, np.where(m_rsi>70, -0.25, 0))
    f['resonance'] = res_d + res_w + res_m
    ...
    st_sig = np.full(n, 0)
    if n >= 14:
        upper = (oh[13]+ol[13])/2 + 3*atr14[13]
        lower = (oh[13]+ol[13])/2 - 3*atr14[13]
        st_sig[13] = 1
        for i in range(14, n):
    ...
    f['supertrend'] = st_sig.astype(float)
    ...
    # ── 8. 成交量异常 ───────────────────────────────
    vol_ma20 = _ma(ov, 20)
    vol_drought = np.full(n, 0.0)
    for i in range(19, n):
        mn = np.min(ov[max(0,i-19):i+1])
        denom = vol_ma20[i]-mn
    ...
    for i in range(19, n):
        mn = np.mean(ov[max(0,i-19):i+1])
        vol_burst[i] = ov[i]/(mn+1e-10)
    f['vol_burst'] = vol_burst

    # Money flow ratio (simplified)
    mf_ratio = np.full(n, 1.0)
    tp = (oh+ol+o)/3; mf = tp*ov
    for i in range(14, n):
        s = max(0, i-13)
        w_tp = tp[s:i+1]
        prev_tp = tp[s-1] if s > 0 else tp[s]
        mask_pos = w_tp > prev_tp
        mask_neg = w_tp < prev_tp
        pos = np.sum(mf[s:i+1][mask_pos])
        neg = np.sum(mf[s:i+1][mask_neg])
...
for mname, cache_dir in markets:
    ...
    for i, fname in enumerate(files):
        try:
            ...
            feats, dates = extract_factors(df, max_bars=1500)
...
for mname, df in all_data.items():
    ...
    print(f'\n  {mname}: {len(recent)} 条, {recent["symbol"].nunique()} 只, 近2年数据')
    ...
    for col in FACTOR_COLS:
        ...
        if not np.isnan(ric):
            ...
            market_ics[col] = {
                'ic': ic, 'rank_ic': ric, 'monthly': monthly,
                'win_rate': wr, 'n_months': len(monthly)
            }

    sorted_f = sorted(market_ics.items(), key=lambda x: abs(x[1]['rank_ic']), reverse=True)
    ic_results[mname] = market_ics
    print(f'    Top15 by |RankIC|:')
    for i, (name, s) in enumerate(sorted_f[:15], 1):
        stars = '⭐' * max(1, int(abs(s['rank_ic'])*40))
        print(f'    {i:2d}. {name:22s} IC={s["ic"]:+.4f}  RIC={s["rank_ic"]:+.4f} {stars}  月胜={s["win_rate"]:.0%}')
...
for mname, df in all_data.items():
    ...
    print(f'\n  {mname} 有效因子(|RankIC|>=0.02): {len(good)} 个')
    ...
    for col, stats in good.items():
        ...
        if len(v) > 2 and np.std(v) > 1e-10:
            vals_norm = np.zeros(len(vals))
            vals_norm[mask] = (v - np.mean(v)) / (np.std(v) + 1e-10)
            fused += (abs(stats['rank_ic']) / tw) * vals_norm
    ...
    valid = valid.copy()
    try:
        valid['q'] = pd.qct(recent['fusion'].dropna(), 5, labels=['Q1(弱)','Q2','Q3','Q4','Q5(强)'], duplicates='drop')
        grp = valid.groupby('q')['fwd_ret_5d'].mean()
        if len(grp) >= 2:
            spread = grp.iloc[-1] - grp.iloc[0]
            print(f'  分层Spread: {spread:+.4f}')
    ...
    print(f'  月度胜率: {wr:.0%}  ({sum(1 for x in monthly if x>0)}/{len(monthly)}月)')
...
if len(all_data) >= 2:
    ...
    print(f'  跨市场共同有效因子: {len(common)} 个')
    for f in sorted(common):
        ...
        print(f'    {f:22s} avg_RIC={avg:+.4f} {stars}')
...
for mname, ics in ic_results.items():
    rows = [{'factor':k, 'market':mname, 'IC':v['ic'], 'RankIC':v['rank_ic'],
             'absRankIC':abs(v['rank_ic']), 'win_rate':v['win_rate'], 'n_months':v['n_months']}
    ...
    if rows:
        ...
        print(f'  ic_{mname.lower()}.csv ({len(rows)} 因子)')
